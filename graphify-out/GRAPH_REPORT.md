...
Cohesion: 0.38
Nodes (7): Path, _detect_roll_type(), _detect_shots(), _process_video(), Detect shots in a video and assemble Shot objects with aligned transcripts., Infer 'a-roll', 'b-roll', or 'unknown' from parent directory name., Detect shots in a video using PySceneDetect ContentDetector. Args: video_path:…

### Community 3 - "preprocess.py"
Cohesion: 0.28
Nodes (8): _normalize_word_text(), Pre-processing pipeline: scene detection + transcription. Walks an input…, Collapse whitespace in a transcribed word while keeping punctuation., Render a token stream into readable caption/transcript text., Return word timestamps whose midpoint falls within the shot window., _words_for_shot(), _words_to_text(), WordTimestamp
...
### Community 6 - "preprocess_footage"
Cohesion: 0.50
Nodes (4): _log(), preprocess_footage(), Pre-process a directory of footage into a FootageIndex. Walks ``input_dir``…, Emit progress to stderr.

### Community 7 - "_transcribe_words"
Cohesion: 0.67
Nodes (3): Transcribe a video and return word-level timestamps. Args: model: A pre-loaded…, _transcribe_words(), WhisperModel
...
- **Why does `preprocess_footage()` connect `preprocess_footage` to `_process_video`, `preprocess.py`, `run_editor_openrouter`, `_transcribe_words`?**
